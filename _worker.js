...
const httpPortSets = new Set([443, 8443, 2053, 2096, 2087, 2083]);
...
function createVlessSub(userIdPath, hostName) {
	...
	const commonHttpUrls = `?encryption=none&security=tls&sni=${hostName}&fp=random&type=ws&host=${hostName}&path=%2F%3Fed%3D2048#`;

	const result = userIDArray.flatMap((userID) => {
		const httpConfig = Array.from(httpPortSet).flatMap((port) => {
			if (!hostName.includes('pages.dev')) {
				const urlPart = `${hostName}-HTTP-${port}`;
				const mainVlessHttp = atob(pt) + '://' + userID + atob(at) + hostName + ':' + port + commonHttpUrl + urlPart;
				return proxyIPs.flatMap((proxyIP) => {
					const secondaryVlessHttp = atob(pt) + '://' + userID + atob(at) + proxyIP + ':' + port + commonHttpUrl + urlPart + '-' + proxyIP + '-' + atob(ed);
					return [mainVlessHttp, secondaryVlessHttp];
				});
			}
			return [];
		});

		const httpConfigs = Array.from(httpPortSets).flatMap((port) => {
			const urlPart = `${hostName}-HTTPS-${port}`;
			const mainVlessHttps = atob(pt) + '://' + userID + atob(at) + hostName + ':' + port + commonHttpUrls + urlPart;
			return proxyIPs.flatMap((proxyIP) => {
				const secondaryVlessHttps = atob(pt) + '://' + userID + atob(at) + proxyIP + ':' + port + commonHttpUrls + urlPart + '-' + proxyIP + '-' + atob(ed);
				return [mainVlessHttps, secondaryVlessHttps];
			});
		});
