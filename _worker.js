...
function getVlessConfig(userIDs, hostName) {
  ...
  const output = userIDArray
    .map((userID) => {
      ...
      const vlessSec =
        atob(pt) + "://" + userID + atob(at) + proxyIP + commonUrlPart;
...
function createVlessSub(userIdPath, hostName) {
  const userIDArray = userIdPath.includes(",")
    ? userIdPath.split(",")
    : [userIdPath];
